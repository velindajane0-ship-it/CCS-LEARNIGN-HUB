...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'login') {
    ...
    $stmt = $conn->prepare("SELECT id, full_name, password FROM instructor WHERE email = ? LIMIT 1");
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            $msg = "Invalid password."; $msg_type = "danger";
        }
    } else {
        $msg = "No faculty account found with that email."; $msg_type = "danger";
    }
    $stmt->close();
}
?>
...
<head>
    ...
    <title>Faculty Login | JRMSU Portal</title>
    ...
    <div class="w-full max-w-md relative z-10">
        <div class="text-center mb-8">
            <div class="inline-flex items-center justify-center w-20 h-20 bg-jrmsuGold rounded-3xl shadow-2xl mb-4 transform -rotate-6 transition-transform hover:rotate-0 duration-300">
                <i class="fas fa-university text-jrmsuNavy text-4xl"></i>
            </div>
            <h1 class="text-white text-2xl font-bold tracking-tight">JRMSU <span class="text-jrmsuGold">Faculty</span></h1>
            <p class="text-white/60 text-sm mt-1">Jose Rizal Memorial State University</p>
        </div>
        ...
        <div class="bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-white/10">
            <div class="bg-jrmsuNavy p-6 text-center border-b-4 border-jrmsuGold">
                <h2 class="text-white font-bold text-lg uppercase tracking-widest">Secure Access</h2>
            </div>
                    ...
                    <div class="pt-2">
                        <button type="submit" id="submitBtn" 
                            ...
                            <span id="btnText" class="flex items-center gap-2">
                                SIGN IN TO PORTAL <i class="fas fa-arrow-right text-xs group-hover:translate-x-1 transition-transform"></i>
                            </span>
        ...
        <p class="text-center mt-8 text-white/40 text-[10px] font-medium uppercase tracking-[0.2em]">
            &copy; 2026 JRMSU Learning Management System
        </p>
    </div>
